...
df<-df[!(df$date.time>= "2016-03-01 00:00:00"),]
...
endDate <- as.POSIXct("2016-03-01 00:00:00",format="%Y-%m-%d %H:%M:%S", tz="HST")
...
# select days and transform hourly means to bi-weekly means based on 84h interval (=1/2 week)
half.wks.dates<-hour.temp.means[1][seq(1, nrow(hour.temp.means), 84), ]
half.wks.dates<-as.data.frame(half.wks.dates[1:length(half.wks.dates)-1])
colnames(half.wks.dates)<-"Date"

half.wks.mean<-.colMeans(hour.temp.means$Water.temp, 84, length(hour.temp.means$Water.temp)/84)

half.week.temps<-cbind(half.wks.dates,half.wks.mean)

## Now collapse to half-weekly temperature
# Create temporary tempmatrix
tempmatrix <- hour.temp.means$Water.temp
...
temp.halfwk.mat<-cbind.data.frame(xi3,temperature_halfwk)
