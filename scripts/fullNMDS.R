...
groups2<-c("Lilipuna-C", "Reef 14-C", "Lilipuna-D", "Reef 14-D")
colors=c(
  "steelblue2", #C Lilipuna
  "mediumpurple1", #C Reef 14
  "tan", # D Lilipuna
  "coral") # D Reef 14
...
with(plot, lines(NMDS.mean[c(1:4),2], NMDS.mean[c(1:4),3], lwd=1.5, col =colors[1])) # Lil C
with(plot, lines(NMDS.mean[c(5:8),2], NMDS.mean[c(5:8),3], lwd=1.5, col =colors[2])) # R14 C
with(plot, lines(NMDS.mean[c(9:12),2], NMDS.mean[c(9:12),3], lwd=1.5, col =colors[3])) # Lil D
with(plot, lines(NMDS.mean[c(13:16),2], NMDS.mean[c(13:16),3], lwd=1.5, col =colors[4])) # R14 D
...
legend("topright", legend=groups2, inset=c(0.045, -0.02), x.intersp=0.8, y.intersp=0.5, cex=0.7, pch=16, col=colors, pt.cex=1, bty="n")
