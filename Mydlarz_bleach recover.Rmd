...
![**Figure 1.**. Multivariate analyses of organisms and their response to environmental change and a conceptual diagram of coral physiology and immunity responses during heat stress.](figures/Fig 1. Infographic_traits and immuno_alt color.jpg){ width=80%}
...
![**Figure 2 (partial).** A bleached *Montipora capitata* coral. (PC: CB Wall)](figures/Montipora bleached.jpg){ width=30%}
...
# all  data from all time points (physio and immuno, qPCR)
physimmun<-read.csv("data/Gates_Mydlarz_20142016.nolab_physimmun.csv", header=T, na.string=NA) #physimmuno data
qPCR.data<-read.csv("data/Gates_Mydlarz_20142016_nolab_qPCR.csv", header=T, na.string=NA) #qPCR data

# qPCR data added with immuno, phys
# no data pre 2014 bleaching
# negative values (assay NA, low read, outliers) removed
data<-read.csv("data/Gates_Mydlarz_20142016_nolab_ALL_DATA.csv", header=T, na.string=NA) 
...
text(0.21, -0.32, "2D Stress = 0.21", cex=0.7, col="black")
...
text(0.35, -0.42, "2D Stress = 0.14", cex=0.7, col="black")
...
fig.df.phys$int<-factor(fig.df.phys$int, levels=c("C.Lilipuna", "C.Reef 14", "D.Lilipuna", "D.Reef 14"))
...
SOD.fig<-ggplot(data=fig.df.immuno.comp, aes(x=Period, y=SOD/10^3, group=int, color=int)) + geom_errorbar(aes(ymin=SOD/10^3-SOD.SE/10^3, ymax=SOD/10^3+SOD.SE/10^3), size=.4, width=0, position=pd) +
  ...
  ylab(expression(paste("SOD activity", ~(Units~SOD~mg~prot^-1), sep=""))) +
  scale_color_manual(values=c(color.scheme.immuno),
                     name= "Symbiont:Site",
                     breaks=break.points.immuno,
                     labels=legend.names.immuno) +
...
Phys.figs
dev.copy(pdf, "figures/Phys.figs.pdf", encod="MacRoman", height=5, width=7)
dev.off()
```
...
![**Figure 5.**.  Schematic of coral responses to repeated bleaching and recovery.](figures/Fig 5. schematic4panel.jpg){ width=100%}
