...
k=1; lwd=1 # k-day moving averages
plot(mean ~ Date, TIMESPAN_mean, type="n", ylab="Temp. (°C) range", ylim=c(0, 3), xaxt="n", xlab="")
mtext(expression(bold("A")), 2, adj=4.5, las=1, padj=-8)
axis.Date(1, at=seq(min(TIMESPAN_mean$Date), max(TIMESPAN_mean$Date), by="1 mon"), format="%b '%y")
legend("topright", lty=1, col=c(reefcols[1:2], "darkgray"), legend=c("Reef 14","Lilipuna"), lwd=2, bty="n")
...
data<-read.csv("Reef survey 2014-2015.csv") #import your data file here
...
df.fig<-cbind(df.m, dfSD[c(4:11,0)]); colnames(df.fig) <-c("Habitat", "Site", "Status", "coral.cover", "coral.pigmented", "coral.pale", "coral.white", "MC", "MC.pigmented", "MC.pale", "MC.white", "coral.SD", "coral.pigSD", "coral.blSD", "coral.whiteSD", "MCSD", "MC.pigSD", "MC.blSD", "MC.whiteSD"); df.fig
