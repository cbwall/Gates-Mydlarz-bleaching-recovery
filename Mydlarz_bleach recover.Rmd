...
annual.temps<-rbind(read.csv("temp_2014.csv"), 
                    read.csv("temp_2015.csv"), 
                    read.csv("temp_2016.csv"))


annual.temps<-annual.temps[!c(annual.temps$X.YY==2014 & annual.temps$MM<= 6),] #start June 2014
annual.temps<-annual.temps[!c(annual.temps$X.YY==2016 & annual.temps$MM > 2),] #end February 2016
...
*Temp and Light plot*
```{r, temp and light plot, message=FALSE, results="hide"}
...
axis.Date(1, at=seq(min(annual.day.temps$Date), max(annual.day.temps$Date), by="2 mon"), 
          format="%b '%y", cex.axis=0.6, cex.lab=0.6)
legend("topright", lty=1, col=c("gray 80", reefcols2[c(1,2)]), legend=c("NOAA-HIMB", "Lilipuna", "Reef 14"), seg.len= 1.5, cex=0.6, lwd=1, bty="n", inset=c(0.1, 0), y.intersp = 1.3)
...
plot(mean ~ Date, TIMESPAN_mean, type="n", ylab=(expression(paste("DLI (mol photons", ~m^-2, ~d^-1, ")", sep=""))), xaxt="n", xlab="", cex.lab=0.7, ylim=c(0,50), cex.axis=0.7)
axis.Date(1, at=seq(min(TIMESPAN_mean$Date), max(TIMESPAN_mean$Date), by="1 mon"), format="%b '%y", cex.axis=0.7, cex.lab=0.7)
...
with(na.omit(data.frame(date=Lil.L5$Date, dli=rollmean(Lil.L5$dli, k, fill=NA))), lines(date, dli, col=reefcols2[1], lwd=lwd))
...
with(na.omit(data.frame(date=Rf14.L5$Date, dli=rollmean(Rf14.L5$dli, k, fill=NA))), lines(date, dli, col=reefcols2[2], lwd=lwd))
...
dev.copy(pdf, "figures/supplement/PAR.pdf", width=7, height=3)
dev.off()
```
...
### 2014-2016 B/R
```{r, ecology, fig.dim=c(6,6), fig.align='center', results="hide"}
...
eco.surv<-subset(df, select = c(-coral.pale, -coral.white, -MC.pale, -MC.white))
...
df.fig<-cbind(df.m, dfSD[c(4:9,0)]); colnames(df.fig) <-c("Site", "Status", "Period", "coral.cover", "coral.pigmented", "totalcoral.bleach", "MC", "MC.pigmented", "totalMC.bleach", "coral.SD", "coral.pigSD", "coral.blSD", "MCSD", "MC.pigSD", "MC.blSD"); df.fig
...
text.formatting<-theme(text=element_text(size=8),
...
FigBlMC <- ggplot(df.fig, aes(x=Period, y=totalMC.bleach, group=Site, color=Site)) + 
  ...
  xlab(ecol.labs) +
  scale_color_manual(values=reefcols2) +
  ylab(expression(bold(paste(~Bleached, bolditalic(" M. capitata")~cover, sep="")))) +
  scale_y_continuous(limits=c(0,1.05),oob = rescale_none)+ 
  scale_x_discrete(name ="Year and Event", 
...
Ecol.Fig<-plot_grid(FigMC, FigBlMC,
     labels=c('A', 'B'), label_size=10, hjust=-3, vjust= 3, ncol=1, nrow=2);
Ecol.Fig

##### save the figure and export to directory? ####
dev.copy(pdf, "figures/Ecol.Fig_col.pdf", width=4, height=7)
dev.off()
```
...
zoox.fig<-ggplot(data=fig.df.phys, aes(x=Period, y=zoox/10^6, group=int, color=int)) + geom_errorbar(aes(ymin=zoox/10^6-zoox.SE/10^6, ymax=zoox/10^6+zoox.SE/10^6), size=.6, width=0, position=pd) +
  ...
  ylab(expression(paste(~italic("symbionts") ~(10^6~cells~cm^-2), sep=""))) +
...
```{r, univariate figures, fig.align='center', results="hide"}

# legend plot
fig.legend.phys<-ggplot(data=fig.df.phys, aes(x=Period, y=zoox, group=int, color=int)) +
...
fig.legend.immun<-ggplot(data=fig.df.immuno.comp, aes(x=Period, y=SOD, group=int, color=int)) +
  geom_point(position=pd, size=3) +
  coord_cartesian(ylim=c(0, 1)) +
  scale_color_manual(values=c(color.scheme.immuno),
                     name= "Clade:Site",
...
```{r, full NMDS with sites periods and dom interaction, fig.show='hide', results='hide', message=FALSE}
...
##  Run MANOVA
all.Manova<-adonis2(resp.fullNMDS~Period*Site*dom, data=fullNMDS,permutations=1000, 
                          method="bray", sqrt.dist = TRUE)


##### test dispersion using PERMDISP
disp <- betadisper(vegdist(resp.fullNMDS, method = "bray"), fac.fullNMDS$Period)
...
######## Period:Site:Symb
disp.3way <- betadisper(vegdist(resp.fullNMDS, method = "bray"), fac.fullNMDS$Period:fac.fullNMDS$Site:fac.fullNMDS$dom)

#test for differences
anova(disp.3way)

## Permutation test for 3-way interaction pairwise comparisons
disp.3way.table<-permutest(disp.3way, pairwise = TRUE)
#capture.output(disp.3way.table,file="test.csv")


############ SIMPER analysis
# tests similarity percentages, using the contribution of species (or metrics) to the community Bray-Curtis dissimilarity. Finds average contributions of species to overal Bray-Curtis, function displays most important groups that contribute at least 70% of difference between groups


# test for Period effects
sim.period<-with(fac.fullNMDS, simper(resp.fullNMDS, Period, permutations=1000))
summary(sim.period, ordered=TRUE)
# Bl1-Rec1: PPO
# Bl2-Rec2: PPO
# Bl1-Bl2: zoox, SOD, CAT, AFDW, chla, chlacell
# Rec1-Rec2: CAT

# test for site effects
sim.site<-with(fac.fullNMDS, simper(resp.fullNMDS, Site, permutations=1000))
summary(sim.site, ordered=TRUE)
# largest effects on zoox, CAT, AFDW


# test for Symbiont (dom) effects
sim.dom<-with(fac.fullNMDS, simper(resp.fullNMDS, dom, permutations=1000))
summary(sim.dom, ordered=TRUE)
# largest differences by chlcell, zoox



###

allNMDS<-metaMDS(resp.fullNMDS, distane='bray', k=2, trymax=100) 
stressplot(allNMDS)
