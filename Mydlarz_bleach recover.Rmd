...
![**Figure 1.**. Multivariate analyses of organisms and their response to environmental change and a conceptual diagram of coral physiology and immunity responses during heat stress.](figures/execute code/Fig 1. Infographic_traits and immuno_alt color.jpg){ width=80%}
...
![**Figure 2 (partial).** A bleached *Montipora capitata* coral. (PC: CB Wall)](figures/execute code/Montipora bleached.jpg){ width=30%}
...
These show a maximum sustained DHW in 2014 of 7.1 (peaked from 04 Oct - 19 Nov). DHW began accumulating 2014-08-30. In 2015 10.2 DHW were sustained (peaked 12 - 19 Nov), with DHW accumulating 2015-07-01. 
  
```{r, results="hide", fig.dim=c(5,5), fig.cap="**Figure NA.** Degree Heating Weeks over 2014-2016 (data reported, figure not in mansucript)."}
...
#set NA to zero for DHW
KBay.DHW[is.na(KBay.DHW)] <- 0
...
Temperature plots for logger temp at each site (color, top) and temperatures for the 2 bleaching years (2014-2015) (2015-2016).   These plots, along with the benthic data (futher down) constitute **Figure 1**.
```{r, temp plot, message=FALSE, results="hide", fig.dim=c(7,3), fig.show='hold', fig.cap="**Figure 1 (partial)**. Temperatures in Kāne'ohe Bay and the two reefs sites during repeated bleaching and recovery periods"}
...
dev.copy(pdf, "figures/execute code/Temp.2014_2016_col.pdf", width=7, height=3)
dev.off()
...
#add DHW for same periods
KBay.DHW.2014<-KBay.DHW[(KBay.DHW$half.wk.dates< "2015-03-01 00:00:00"),]
KBay.DHW.2015<-KBay.DHW[(KBay.DHW$half.wk.dates> "2015-01-01 00:00:00"),]

### 2014-bleaching
k=1; lwd=0.7 # k-day moving averages
plot(mean ~ Date, Temp.event1, type="l", ylab="Mean daily temp. (°C)", cex.lab=0.6, cex.axis=0.6, ylim=c(19, 31), xlab="", xaxt="n", col=annual.color[1])
...
### 2015 bleaching
par(new=T)
plot(mean ~ Date, Temp.event2, type="l", cex.lab=0.6, cex.axis=0.6, ylim=c(19, 31), xlab="", xaxt="n", ylab="", col=annual.color[2])
...
# add DHW 2014
par(new=T)
plot(KBay.DHW.2014$half.wk.dates, KBay.DHW.2014$DHW, type="l", lty=1, xaxt = "n", yaxt = "n",
     ylab="",xlab="", col="gray80", ylim=c(0,20))

# add DHW 2015
par(new=T)
plot(KBay.DHW.2015$half.wk.dates, KBay.DHW.2015$DHW, type="l", lty=1, xaxt = "n", yaxt = "n",
     ylab="",xlab="", col="black", ylim=c(0,20))
axis(side=4, cex.axis=0.6)
mtext("DHW (°C)", side=4, cex=0.6)

dev.copy(pdf, "figures/execute code/Temp.AnnualHeating_col2.pdf", width=7, height=4)
dev.off()
```
...
```{r, light data, results="hide", fig.dim=c(7,3), fig.cap="**Supplementary Figure 1.**  Light conditions at the two reefs through time."}
...
dev.copy(pdf, "figures/execute code/PAR.pdf", width=7, height=3)
dev.off()
```
...
### 2014-2016 benthic
```{r, ecology, fig.dim=c(7,4), fig.align='center', results="hide", fig.cap="**Figure 2 (partial)**. *Montipora capitata* total cover and bleached cover in Kāne'ohe Bay at the two reefs sites during repeated bleaching and recovery periods."}
...
##############
# all corals
Fig.cor <- ggplot(data=df.fig, aes(x=Period, y=coral.cover*100,  group=Site, color=Site)) +
  geom_errorbar(aes(ymin=coral.cover*100-coral.SD*100, ymax= coral.cover*100+coral.SD*100), 
                width=0, position=pd) +
  geom_line(position=pd, size=.6) +
  geom_point(position=pd, size=1) +
  ylab("% benthic cover") +
  scale_color_manual(values=reefcols2) +
  ggtitle(expression(bold(paste(~"Coral cover"), sep=""))) +
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_y_continuous(limits=c(0,105),oob = rescale_none)+ 
  scale_x_discrete(name ="Year and Period", 
      labels=ecol.labs)+
  text.formatting

# MC corals
FigMC <- ggplot(df.fig, aes(x=Period, y=MC*100, group=Site, color=Site)) + 
  geom_errorbar(aes(ymin=MC*100-MCSD*100, ymax= MC*100+MCSD*100), width=0, position=pd) +
  geom_line(position=pd, size=.6) +
  geom_point(position=pd, size=1) +
  ylab("") +
  scale_color_manual(values=reefcols2) +
...
# bleached MC coral
FigBlMC <- ggplot(df.fig, aes(x=Period, y=totalMC.bleach*100, group=Site, color=Site)) + 
  geom_errorbar(aes(ymin=totalMC.bleach*100-MC.blSD*100, ymax= totalMC.bleach*100+MC.blSD*100), 
...
# get legend
ecol.legend<-get_legend(FigBlMC)
...
Ecol.Fig<-plot_grid(Fig.cor + theme(legend.position = "none"),
                    FigMC + theme(legend.position = "none"), 
                    FigBlMC + theme(legend.position = "none"),
                    ecol.legend,
     labels=c('a', 'b', 'c', ""), label_size=8, hjust=-3, vjust= 3, ncol=4, nrow=1, rel_widths = c(1,1,1,0.3))
Ecol.Fig

##### save the figure and export to directory? ####
dev.copy(pdf, "figures/execute code/Ecol.Fig_col2.pdf", width=7, height=4)
dev.off()
```
  
**Statistics for bleaching and recovery bethic data.**   

Used Kruskal Wallis nonparamtetirc tests to examine  effects of bleaching on *total coral cover*, *total Montipora cover*, and *BLEACHED Montipora cover*.  Used separate dataframes for non-parametric tests, with all data (4 periods) just bleaching, and just recovery (2 periods each). Tested effects of two sites, but only significant for bleached *M. capitata.*    
(1) *total coral cover*: period effect, difference between bleaching years (p=0.029)  
(2) *total M capitata cover*: period effect, difference between bleaching (p=0.003) & recoverey (p=0.033) years 
(3a) *bleached M capitata*: period effect, difference between bleaching years (p<0.001)  
(3b) *bleached M capitata*: site effect, sites different in bleaching (p=0.001), and recovery (p=0.013) years  
```{r, B and R Montipora, results='hide'}
##################
###############
##########
#####
...
# total  cover
################
ecology.bl.df<-ecology[(ecology$Status=="Bleaching"),]
ecology.rec.df<-ecology[(ecology$Status=="Recovery"),]

kruskal.test(coral.cover ~ Period, data = ecology) # Period effect
kruskal.test(coral.cover ~ Period, data = ecology.bl.df) # bleaching different between 2 BL periods
kruskal.test(coral.cover ~ Period, data = ecology.rec.df) # NO different between 2 REC periods
kruskal.test(coral.cover ~ Site, data = ecology) # no Site effect

################
# total MC cover
################
kruskal.test(MC ~ Period, data = ecology) # Period effect
kruskal.test(MC ~ Period, data = ecology.bl.df) # bleaching different between 2 BL periods
kruskal.test(MC ~ Period, data = ecology.rec.df) # different between 2 REC periods
kruskal.test(MC ~ Site, data = ecology) # no Site effect


################
# bleached MC
################
kruskal.test(total.MC.bleach ~ Period, data = ecology) # Period effect
kruskal.test(total.MC.bleach ~ Period, data = ecology.bl.df) # bleaching different between 2 BL periods
kruskal.test(total.MC.bleach ~ Period, data = ecology.rec.df) # no diff

kruskal.test(total.MC.bleach ~ Site, data = ecology) #  Site effect
kruskal.test(total.MC.bleach ~ Site, data = ecology.bl.df) #  Site effect
kruskal.test(total.MC.bleach ~ Site, data = ecology.rec.df) #  Site effect
...
stressplot(allNMDS)
dev.print(pdf, "output/Trajectory.NMDS.stressplot.pdf", height=5, width=5)

``` 
  
...now the 4 part convex hulls for coral trajectories.
```{r, full NMDS trajectory, results='hide', message=FALSE, fig.dim=c(7,7), fig.cap="**Figure 3.** Non-metric multidimensional scaling (NMDS) analyses of coral performance envelopes during bleaching stress and post-bleaching recovery."}
...
with(plot, points(NMDS.mean[1,2], NMDS.mean[1,3], bg="white", col=colors[1], pch=21, cex=1, lwd=1)) # Lil C
with(plot, points(NMDS.mean[4,2], NMDS.mean[4,3], col=colors[1], pch=17, cex=1, lwd=1)) # Lil C
with(plot, points(NMDS.mean[5,2], NMDS.mean[5,3], bg="white", col=colors[2], pch=21, cex=1, lwd=1)) # R14 C
with(plot, points(NMDS.mean[8,2], NMDS.mean[8,3], col=colors[2], pch=17, cex=1, lwd=1)) # R14 C
with(plot, points(NMDS.mean[9,2], NMDS.mean[9,3], bg="white", col=colors[3], pch=21, cex=1, lwd=1)) # Lil D
with(plot, points(NMDS.mean[12,2], NMDS.mean[12,3], col=colors[3], pch=17, cex=1, lwd=1)) # Lil D
with(plot, points(NMDS.mean[13,2], NMDS.mean[13,3], bg="white", col=colors[4], pch=21, cex=1, lwd=1)) # R14 D
...
# add labels
text(-0.14, -0.05, "Bleaching \n 2014", cex=0.8, col="gray50")
text(-0.14, 0.12, "Bleaching \n 2015", cex=0.8, col="gray50")
text(0.14, 0.05, "Recovery \n 2015", cex=0.8, col="gray50")
text(0.06, -0.07, "Recovery \n 2016", cex=0.8, col="gray50")
text(0.12, -0.12, "2D Stress = 0.20", cex=0.8, col="black")
...
dev.copy(pdf, "figures/execute code/1panel_allNMDS.pdf", height=5.5, width=6, useDingbats=FALSE)
dev.off() 

```

```{r, control plot example, eval=FALSE, include=FALSE}

########## ########## ########## ########## ########## ########## ########## 
########## control plot of centroid distance (not useful since NMDS is rank-based, so distance non-informative)

NMDS.mean.period=aggregate(allNMDS.df[,1:2],list(group=allNMDS.df$Period), mean)
# a2 + b2 = c2. Distance is (|x1|+|x2|)^2 + sqrt(|y1|+|y2|)^2 = sqrt(c)
B1.R1<- sqrt(
          (abs(NMDS.mean.period[1,2]) + abs(NMDS.mean.period[2,2]))^2 + #x
          (abs(NMDS.mean.period[1,3]) + abs(NMDS.mean.period[2,3]))^2)  #y

R1.B2<- sqrt(
          (abs(NMDS.mean.period[2,2]) + abs(NMDS.mean.period[3,2]))^2 + #x
          (abs(NMDS.mean.period[2,3]) + abs(NMDS.mean.period[3,3]))^2)  #y

B2.R3<- sqrt(
          (abs(NMDS.mean.period[3,2]) + abs(NMDS.mean.period[4,2]))^2 + #x
          (abs(NMDS.mean.period[3,3]) + abs(NMDS.mean.period[4,3]))^2)  #y

centroid.dist<-c(B1.R1,R1.B2,B2.R3)
time=c("B1-R1", "R1-B2", "B2-R2")
control.plot<-as.data.frame(cbind(centroid.dist,time))
control.plot$centroid.dist=as.character(control.plot$centroid.dist)
control.plot$time<-factor(control.plot$time, levels=c("B1-R1", "R1-B2", "B2-R2"))

#### mean period control plot
#plot(control.plot$centroid.dist~control.plot$time, type='b', ylim=c(-0.05,0.15), col="orchid", pch=16, xlab="time", xaxt="n", ylab="centroid distance", cex.axis=0.8)

#axis(1, at=control.plot$time, labels=control.plot$time, cex.axis=0.8, cex=0.8)
#abline(h = 0, lty = "dotted")


#####  control plots, try for each level
LilC<-NMDS.mean[1:4,]
R14C<-NMDS.mean[5:8,]
LilD<-NMDS.mean[9:12,]
R14D<-NMDS.mean[13:16,]

## LilC
LilC.1<- sqrt(
          (abs(LilC[1,2]) + abs(LilC[2,2]))^2 + #x
          (abs(LilC[1,3]) + abs(LilC[2,3]))^2)  #y
LilC.2<- sqrt(
          (abs(LilC[2,2]) + abs(LilC[3,2]))^2 + #x
          (abs(LilC[2,3]) + abs(LilC[3,3]))^2)  #y
LilC.3<- sqrt(
          (abs(LilC[3,2]) + abs(LilC[4,2]))^2 + #x
          (abs(LilC[3,3]) + abs(LilC[4,3]))^2)  #y
LilC.centroid.dist<-c(LilC.1,LilC.2,LilC.3)


## R14C
R14C.1<- sqrt(
          (abs(R14C[1,2]) + abs(R14C[2,2]))^2 + #x
          (abs(R14C[1,3]) + abs(R14C[2,3]))^2)  #y
R14C.2<- sqrt(
          (abs(R14C[2,2]) + abs(R14C[3,2]))^2 + #x
          (abs(R14C[2,3]) + abs(R14C[3,3]))^2)  #y
R14C.3<- sqrt(
          (abs(R14C[3,2]) + abs(R14C[4,2]))^2 + #x
          (abs(R14C[3,3]) + abs(R14C[4,3]))^2)  #y
R14C.centroid.dist<-c(R14C.1,R14C.2,R14C.3)


## LilD
LilD.1<- sqrt(
          (abs(LilD[1,2]) + abs(LilD[2,2]))^2 + #x
          (abs(LilD[1,3]) + abs(LilD[2,3]))^2)  #y
LilD.2<- sqrt(
          (abs(LilD[2,2]) + abs(LilD[3,2]))^2 + #x
          (abs(LilD[2,3]) + abs(LilD[3,3]))^2)  #y
LilD.3<- sqrt(
          (abs(LilD[3,2]) + abs(LilD[4,2]))^2 + #x
          (abs(LilD[3,3]) + abs(LilD[4,3]))^2)  #y
LilD.centroid.dist<-c(LilD.1,LilD.2,LilD.3)


## R14D
R14D.1<- sqrt(
          (abs(R14D[1,2]) + abs(R14D[2,2]))^2 + #x
          (abs(R14D[1,3]) + abs(R14D[2,3]))^2)  #y
R14D.2<- sqrt(
          (abs(R14D[2,2]) + abs(R14D[3,2]))^2 + #x
          (abs(R14D[2,3]) + abs(R14D[3,3]))^2)  #y
R14D.3<- sqrt(
          (abs(R14D[3,2]) + abs(R14D[4,2]))^2 + #x
          (abs(R14D[3,3]) + abs(R14D[4,3]))^2)  #y
R14D.centroid.dist<-c(R14D.1,R14D.2,R14D.3)

#make df
cent.df<-as.data.frame(cbind(time,LilC.centroid.dist,R14C.centroid.dist,LilD.centroid.dist,R14D.centroid.dist))

cent.df$LilC.centroid.dist<-as.character(cent.df$LilC.centroid.dist)
cent.df$R14C.centroid.dist<-as.character(cent.df$R14C.centroid.dist)
cent.df$LilD.centroid.dist<-as.character(cent.df$LilD.centroid.dist)
cent.df$R14D.centroid.dist<-as.character(cent.df$R14D.centroid.dist)
cent.df$time<-factor(cent.df$time, levels=c("B1-R1", "R1-B2", "B2-R2"))

####  control plot

#LilC
plot(cent.df$LilC.centroid.dist~cent.df$time, type='b', ylim=c(0.0,0.2), col=colors[1], lty=2, pch=16,
     xlab="time", xaxt="n", ylab="centroid distance", cex.axis=0.8)

#R14C
par(new=T)
plot(cent.df$R14C.centroid.dist~cent.df$time, type='b', ylim=c(0.0,0.2), col=colors[2], lty=1, pch=16,
     yaxt="n", xaxt="n", ylab="", xlab="")
#LilD
par(new=T)
plot(cent.df$LilD.centroid.dist~cent.df$time, type='b', ylim=c(0.0,0.2), col=colors[3], lty=2, pch=16,
     yaxt="n", xaxt="n", ylab="", xlab="")
#R14D
par(new=T)
plot(cent.df$R14D.centroid.dist~cent.df$time, type='b', ylim=c(0.0,0.2), col=colors[4], lty=1, pch=16,
     yaxt="n", xaxt="n", ylab="", xlab="")
axis(1, at=cent.df$time, labels=cent.df$time, cex.axis=0.8, cex=0.8)
abline(h = 0, lty = "dotted")
legend("topright", legend=groups2, #inset=c(0.03, -0.01), x.intersp=0.6, y.intersp=0.9, 
       cex=0.7, pch=16, col=colors, lty=c(1,3,1,3), seg.len=2, pt.cex=1, bty="n")
```
...
## NMDS plots
NMDS.Lil.BR1<-metaMDS(Manova.Lil.BR1.dat, distance='euclidian', k=2, trymax=100) 
...
## NMDS
NMDS.Lil.BR2<-metaMDS(Manova.Lil.BR2.dat, distance='euclidian', k=2, trymax=100) 
...
## NMDS
NMDS.R14.BR1<-metaMDS(Manova.R14.BR1.dat, distance='euclidian', k=2, trymax=100) 
...
## NMDS
NMDS.R14.BR2<-metaMDS(Manova.R14.BR2.dat, distance='euclidian', k=2, trymax=100) 
...
```{r, 4 panel NMDS plots, fig.dim=c(8,7), results='hide', fig.cap="**Figure 4.** Non-metric multidimensional scaling (NMDS) analyses identify coral physiotypes separated by dominant symbiont type (colors), bleaching-recovery periods (lighter and darker shades), sites (columns), and events (rows)."}
...
### 
NMDS.plot<-ordiplot(NMDS.Lil.BR1, type="n", main=substitute(paste("Lilipuna")), cex.main=1, display="sites", cex.lab=0.8, cex.axis=0.8, ylim=c(-0.15, 0.15), xlim=c(-0.2, 0.2), xaxt="n", xlab="")
...
legend("topright", legend=MDS.lev.leg, inset=c(0.75, 0), x.intersp=0.9, y.intersp=1, cex=0.7, pch=16, col=MDS.colors, pt.cex=1, bty="n")
text(0.15, -0.15, "2D Stress = 0.13", cex=0.7, col="black")
...
NMDS.plot<-ordiplot(NMDS.Lil.BR2, type="n", cex.main=1, display="sites", cex.lab=0.8, cex.axis=0.8, ylim=c(-0.15, 0.15), xlim=c(-0.2, 0.2))
...
legend("topright", legend=MDS.lev.leg, inset=c(0.75, 0), x.intersp=0.9, y.intersp=1, cex=0.7, pch=16, col=MDS.colors, pt.cex=1, bty="n")
text(0.15, -0.15, "2D Stress = 0.14", cex=0.7, col="black")
...
NMDS.plot<-ordiplot(NMDS.R14.BR1, type="n", main=substitute(paste("Reef 14")), cex.main=1, display="sites", cex.lab=0.8, cex.axis=0.8, ylim=c(-0.15, 0.15), xlim=c(-0.2, 0.2), yaxt="n", ylab="", xaxt="n", xlab="")
axis(side = 1, labels = FALSE, tck = -0.05)
axis(side = 2, labels = FALSE, tck = -0.05)
abline(h = 0, lty = "dotted")
abline(v = 0, lty = "dotted")
...
NMDS.plot<-ordiplot(NMDS.R14.BR2, type="n", cex.main=1, display="sites", cex.lab=0.8, cex.axis=0.8, ylim=c(-0.15, 0.15), xlim=c(-0.2, 0.2), yaxt="n", ylab="")
axis(side = 2, labels = FALSE, tck = -0.05)
abline(h = 0, lty = "dotted")
abline(v = 0, lty = "dotted")
points(NMDS.plot, "sites", cex=0.8, pch=16,  col=MDS.colors[groups])
ordihull(NMDS.plot, groups=NMDS.df.R14.BR2$Period.dom, draw="polygon", alpha=100, col=MDS.colors, border=MDS.colors)
text(0.15, -0.15, "2D Stress = 0.19", cex=0.7, col="black")
...
dev.copy(pdf, "figures/execute code/4panel_NMDS.pdf", height=6, width=7, useDingbats=FALSE)
dev.off() 
```
...
zoox.fig<-ggplot(data=fig.df.phys, aes(x=Period, y=zoox/10^6, group=int, color=int)) + geom_errorbar(aes(ymin=zoox/10^6-zoox.SE/10^6, ymax=zoox/10^6+zoox.SE/10^6), size=.4, width=0, position=pd) + 
  geom_line(aes(linetype=Site), position=pd, lwd=0.3) +
  scale_linetype_manual(values=c("solid", "longdash"))+
  geom_point(position=pd, size=0.7, pch=19) +
...
```{r, physiology figures, fig.align='center', results="hide", fig.dim=c(8,5), fig.cap="**Supplementary Figure 2.** Physiological metrics for *M. capitata* corals dominated dominated by *Cladocopium* sp. or *Durusdinium* sp. symbionts (C or D) from two reefs in Kāne‘ohe Bay during repeated bleaching and recovery periods."}

# legend plot
fig.legend<-get_legend(zoox.fig)
...
Phys.figs<-plot_grid( (zoox.fig + theme(legend.position = "none")), 
           ...
           labels = c("a", "b", "c", "d", "e"), ncol = 5, label_size=8, 
           rel_widths = c(1.5, 1.5, 1.5, 1.55, 1.52))

Phys.fig.leg<- plot_grid(Phys.figs, fig.legend, rel_widths = c(10,0.6))

Phys.fig.leg
dev.copy(pdf, "figures/execute code/Phys.figs2.pdf", encod="MacRoman", height=4, width=10)
dev.off()
```

```{r, immunity figures, fig.align='center', results="hide", fig.dim=c(8,5), fig.cap="**Supplementary Figure 3.** Immunity metrics for *M. capitata* corals dominated dominated by *Cladocopium* sp. or *Durusdinium* sp. symbionts (C or D) from two reefs in Kāne‘ohe Bay during repeated bleaching and recovery periods."}
...
Immuno.figs<-plot_grid( (MEL.fig + theme(legend.position = "none")),
           ...
           labels = c("e", "f", "g", "h", "i"), ncol = 5, label_size=8,
           rel_widths = c(1.55, 1.5, 1.5, 1.45, 1.51))

Immuno.fig.leg<- plot_grid(Immuno.figs, fig.legend, rel_widths = c(10,0.6))

Immuno.fig.leg
dev.copy(pdf, "figures/execute code/Immuno.figs2.pdf", encod="MacRoman", height=4, width=10)
dev.off()

##### All grouped figure
Phys.immuno.leg<-plot_grid(Phys.fig.leg, Immuno.fig.leg, ncol=1, rel_widths = c(10,0.6))
Phys.immuno.leg
dev.copy(pdf, "figures/execute code/Phys.Immuno.figs.pdf", encod="MacRoman", height=7, width=10)
dev.off()
```
...
model.data$chlacell<-(model.data$chlacell)^0.25
...
## if want p values for comparisons, lsmeans(full, pairwise~Period)
...
posthoc<-lsmeans(full, ~Period|dom)
multcomp::cld(posthoc, Letters=letters)
...
full<-lm(Y~Period*Site*dom, data=model.data, na.action=na.exclude)
#summary(full)
print(Anova(full, type=2), digits=6)
...
posthoc<-lsmeans(full, ~Period)
contrast(posthoc, method="pairwise", adjust="Tukey")
multcomp::cld(posthoc, Letters=letters)
...
![**Figure 5.**.  Schematic of coral responses to repeated bleaching and recovery.](figures/execute code/Fig 5. schematic4panel.jpg){ width=100%}
