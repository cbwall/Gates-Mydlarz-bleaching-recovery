...
###################
# zoox 
zoox.fig<-ggplot(data=fig.df.phys, aes(x=Period, y=zoox/10^6, group=int, color=int)) + geom_errorbar(aes(ymin=zoox/10^6-zoox.SE/10^6, ymax=zoox/10^6+zoox.SE/10^6), size=.6, width=0, position=pd) + geom_line(aes(linetype=SimpleSite), position=pd) +
...
chla.fig<-ggplot(data=fig.df.phys, aes(x=Period, y=chla, group=int, color=int)) + geom_errorbar(aes(ymin=chla-chla.SE, ymax=chla+chla.SE), size=.6, width=0, position=pd) +
  geom_line(aes(linetype=SimpleSite), position=pd) +
  scale_linetype_manual(values=c("solid", "longdash"))+
  geom_point(position=pd, size=1) +
...
*Period* is the 4 Periods, *Site* is the two reefs, *dom* is the symbiont community (C- or D-dominated)
...
=======
                          method="euclidian")
all.Manova
>>>>>>> d724dd164eab10e0146b4206c9bc47bc87fa9a7a
