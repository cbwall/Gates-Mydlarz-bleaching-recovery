...
```{r, temp and light plot, message=FALSE, results="hide", fig.dim=c(7,3), fig.show='hold', fig.cap="**Figure 1 (partial)**. Temperatures in Kāne'ohe Bay and the two reefs sites during repeated bleaching and recovery periods"}
...
dev.copy(pdf, "figures/execute code/Temp.AnnualHeating_col.pdf", width=7, height=3)
dev.off()
```
...
dev.copy(pdf, "figures/execute code/Ecol.Fig_col.pdf", width=7, height=4)
dev.off()
```
...
text(0.12, -0.12, "2D Stress = 0.20", cex=0.7, col="black")
...
dev.copy(pdf, "figures/execute code/1panel_allNMDS.pdf", height=6, width=6, useDingbats=FALSE)
dev.off() 
...
Phys.figs
dev.copy(pdf, "figures/execute code/Phys.figs.pdf", encod="MacRoman", height=6, width=8)
dev.off()
```
...
Immuno.figs<-plot_grid( (MEL.fig + theme(legend.position = "none")),
          ...
          labels = c("a", "b", "c", "d", "e",""), ncol = 3, label_size=8)
Immuno.figs
dev.copy(pdf, "figures/execute code/Immuno.figs.pdf", encod="MacRoman", height=6, width=8)
dev.off()
