...
ecology<-ecology[!(ecology$Habitat=="Slope" & !ecology$Habitat=="Flat") ,] 
...
##############
# MC corals
FigMC <- ggplot(df.fig, aes(x=Site, y=MC, fill=Status)) + 
  geom_bar(stat="identity", position=position_dodge(), colour="black") +
  geom_errorbar(aes(ymin=MC-MCSD, ymax= MC+MCSD), width=0.0, position=position_dodge(.9)) +
  scale_fill_manual(values=colors) +
  ylab(expression(bold(paste(~bolditalic("M. capitata")~cover, sep="")))) +
  scale_y_continuous(limits=c(0,1.05),oob = rescale_none)+ text.formatting +
  facet_grid(.~groupyear) + 
  theme(axis.title.x = element_blank())

# bleached MC coral
FigBlMC <- ggplot(df.fig, aes(x=Site, y=totalMC.bleach, fill=Status)) + 
  geom_bar(stat="identity", position=position_dodge(), colour="black") +
  geom_errorbar(aes(ymin=totalMC.bleach-MC.blSD, ymax= totalMC.bleach+MC.blSD),  width=0.0,position=position_dodge(.9)) +
  scale_fill_manual(values=colors) +
...
Ecol.Fig<-plot_grid(FigMC, FigBlMC,
     labels=c('A', 'B'), label_size=10, hjust=-6, vjust= 3, ncol=1, nrow=2);
Ecol.Fig

##### save the figure and export to directory? ####
dev.copy(pdf, "figures/Ecol.Fig.pdf", width=5, height=6)
dev.off()
```
...
plot<-ordiplot(allNMDS, type="n", main=substitute(paste("full NMDS")), cex.main=1, display="sites", cex.lab=0.8, cex.axis=0.8, ylim=c(-0.3, 0.3))
